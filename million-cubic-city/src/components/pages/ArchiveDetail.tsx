...
const ArchiveDetail = () => {
  ...
  // Load data
  useEffect(() => {
    // Load title from shared content (synced with Story card)
    const loadedTitle = content[titleKey] || id?.replace(/-/g, ' ').toUpperCase() || 'ARCHIVE';

    // Load detail content
    const loadedContent = content[contentKey] || '';

    // Load media
    const savedMedia = localStorage.getItem(`archive_media_${id}`);

    setTitle(loadedTitle);
    setDetailContent(loadedContent);
    if (savedMedia) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await saveContent(contentKey, detailContent, 'richtext');
      localStorage.setItem(`archive_media_${id}`, JSON.stringify(mediaItems));
    ...
    <div className="min-h-screen bg-gray-900 text-white p-8 pt-24 relative">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          ...
          <div className="mb-8">
            <label className="block text-sm font-medium text-gray-400 mb-4 uppercase tracking-wider">Media Gallery</label>
            <MediaUpload items={mediaItems} onChange={setMediaItems} />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-400 mb-4 uppercase tracking-wider">Detailed Content</label>
            <textarea
              value={detailContent}
              ...
              placeholder="Start writing the archive history..."
            />
          </div>
        </motion.div>
      </div>
